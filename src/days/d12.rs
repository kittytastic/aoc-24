...
pub fn day12_main(_second_part: bool, _extra_args: &Vec<String>){
    let input_str = get_input_file("d12_example4.txt");
    ...
    for y in 0..grid.height(){
        for x in 0..grid.width(){
            ...
            if *visited.at_point(&p) == '.'{
                ...
                let area = dfs_p2(&Point::new(x, y), current_crop, &grid, &mut visited, &mut boundaries);
                println!("Current Crop {}", current_crop);
                let edges = count_corners(&mut boundaries, &grid);
                println!("Current Crop {}  {} * {}", current_crop, area, edges);
...
fn dfs_p2(point: &Point, crop_val: char, grid: &Grid<char>, visited: &mut Grid<char>, boundaries: &mut HashSet<(Point, Direction)>)->usize{
    if *grid.at_point(point) != crop_val {return 0}
    if *visited.at_point(point) != '.' {return 0}
    visited.set(point.get_x(), point.get_y(), 'X');    
    ...
    for d in vec![Direction::Up, Direction::Down, Direction::Left, Direction::Right]{
        if let Some(new_p) = point.step_direction(d, grid.width(), grid.height()){
            ...
            let sub_area = dfs_p2(&new_p, crop_val, grid, visited, boundaries);
            area += sub_area;
        }else{
                boundaries.insert((point.clone(), d));
        }
...
fn count_corners(boundaries: &mut HashSet<(Point, Direction)>, grid: &Grid<char>)->usize{
    ...
    for (point, edge) in sorted_b.iter(){
        ...
        for (new_point, new_edge, touching) in interior_corners{
            if boundaries.contains(&(new_point.clone(), new_edge)){
                ...
                if  !connected_corner{
                    i += 1;
                    println!("[{}] Found corner {}:{:?} -> {}:{:?}", i, point, edge, new_point, new_edge);
                    corner_count += 1;
                }
            }
        }
        ...
        for (new_point, new_edge) in exterior_corners{
            if let Some(p) = new_point{
                if boundaries.contains(&(p.clone(), new_edge)){
                    i += 1;
                    println!("[{}] Found corner {}:{:?} -> {}:{:?}", i, point, edge, p, new_edge);
                    corner_count += 1;
                }
            }
        } 
